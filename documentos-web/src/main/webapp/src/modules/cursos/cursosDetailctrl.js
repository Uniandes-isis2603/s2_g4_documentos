(function(ng){
    ...
    mod.controller('cursosDetailCtrl', ['$scope', '$http', '$state',
        /**
         * @ngdoc controller
        ...
        function($scope,$http,$state) {
            if(($state.params.cursoId !== undefined) && ($state.params.cursoId !==null)){
                /**
                 * @ngdoc function
                 * @name getFotocopiaID
                ...
                $http.get('http://localhost:8080/documentos-web/api/cursos/' + $state.params.cursoId).then(function(response){
                    $scope.currentCurso =response.data;
                });
